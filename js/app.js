...
searchBtn.addEventListener("click", (e) => {
  ...
  if (inputValue.length > 0) {
    ...
    document.getElementById(
      "error-message"
    ).innerHTML = `<p class='text-center p-3 bg-warning'><b>Please enter a city name...</b></p>`;
  }
});

const getValue = (getName) => {
  displayWeather(
    `https://api.openweathermap.org/data/2.5/weather?q=${getName}&units=metric&APPID=${apiKey}`
  );
};
...
const displayWeather = (url) => {
  ...
  fetchget(url).then((data) => {
    ...
    if (data.cod === "404") {
      document.getElementById(
        "error-message"
      ).innerHTML = `<p class='text-center p-3 bg-danger'><b>Please enter a city name...</b></p>`;
    } else {
      ...
      searchResult.innerHTML = `
                ...
                <h3><span>Temperature: ${Math.round(
                  data.main.temp
                )}</span>&deg;C</h3>
                <h1 class="lead fw-bold">Weather: ${data.weather[0].main}</h1>
                <h1 class="lead fw-bold">Temperature Range: ${Math.round(
                  data.main.temp_min
                )}&deg; to ${Math.round(data.main.temp_max)}&deg;</h1>
                `;
    }
  });
};
